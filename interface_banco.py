...
lista_contas = []
def armazena():
    global lista_cliente, nome, cpf, data, us1, sh2, a
...
def confir_c():
    ...
    for i in range(len(lista_cliente)):
        if lista_cliente[i].usuario == us2.get() and lista_cliente[i].senha == sh3.get():
           menu() 
        else:
...
def conf_saque():
    global lista_contas, a
    for i in range(len(lista_contas)):
        if lista_contas[i].cont == a:
...
def conf_transf():
    global lista_contas, transfcont, transfva
...
#teste.bind(update)
#teste.pack()
#teste = Label(loading, text='ospfo')
#teste.pack()
...
b3 = Button(f2, image=btentrar, bd=0, command=lambda: [confir_f()])
...
def conta():
    global cont, a
    cont = Label(f4,background=cinzaback,foreground=fore, font='Arial 20',text="00"+str(randint(1,99)))
...
b6 = Button(f3, image=btentrar, bd=0, command=lambda: [confir_c()])
...
deposit = Button(f7, image=btde, bd=0, command=lambda: conf_depos())
...
transf = Button(f8, image=bttransferir, bd=0, command=update)
...
extrava.place(width=442, height=298, x=272, y=173)
extra = Button(f9, image=btex, bd=0)
